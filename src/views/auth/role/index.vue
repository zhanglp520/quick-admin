...
const handleAuth = (item: Role, done: any) => {
...
const tableActionbar = reactive<Actionbar>({
  ...
  btns: [
    {
      name: '配置权限',
      click(item: Role, done: any) {
        handleAuth(item, done)
      },
    }
  ]
})
...
const active = ref(1)
const dialogProgressRef = ref(null)
const prev = () => {
...
<template>
  ...
  <el-dialog title="角色授权" append-to-body v-model="dialogVisible" width="30%">
    <div style="height:400px">
      <dialog-progress ref="dialogProgressRef" v-if="dialogVisible" :role="role" @active="handleActive"></dialog-progress>
    </div>
    <template #footer>
